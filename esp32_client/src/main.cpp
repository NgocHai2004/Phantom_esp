...
bool syncFromNode1() {
  ...
  // Dùng listJson đã lấy từ pull phase — KHÔNG reconnect lại
  // (tránh race condition Node-1 SPIFFS bị empty khi reconnect)
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    while (entry) {
      ...
      // Kiểm tra file đã có trên Node-1 chưa — dùng listJson từ pull phase
      bool alreadyOnNode1 = false;
      int32_t remSz = getRemoteFileSize(listJson, ename);
      ...
      if (!alreadyOnNode1) {
        ...
        if (sent == esize) {
          ...
          pushed++;
          // Cập nhật listJson giả để tránh push lại trong cùng cycle
          listJson += String(",{\"name\":\"") + ename + "\",\"size\":" + String(esize) + "}";
        } else {
    ...
    Serial.println("[Push] Không còn kết nối Thiết bị A — bỏ qua push");
  }
