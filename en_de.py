...
# Khng dng zipfolder/pyminizip na v pyminizip cn Microsoft Visual C++ Build Tools trn Windows.
# Dng zipfile chun ca Python  nn/gii nn .zfld.

def compress_folder(folder_path, output_zip_path=None, optimize=True, algorithm="zip"):
    """
    Nn ton b folder thnh file .zfld/.zip bng zipfile chun Python.

    Tng thch vi cch gi c:
        compress_folder(str(work_folder), str(zfld_path), optimize=True, algorithm="7z")

    Cc tham s optimize/algorithm c gi  code c khng li,
    nhng bn trong dng ZIP_DEFLATED thay cho zipfolder/pyminizip.
    """
...
def decompress_folder(zip_path, output_dir):
    """
    Gii nn file .zfld/.zip ra output_dir bng zipfile chun Python.

    Tng thch vi cch gi c:
        result_folder = decompress_folder(zfld_tmp, out_d)

    C kim tra zip-slip  trnh file trong zip ghi ra ngoi output_dir.
    """
...
def _phtm_load_public_key(path):
    raw = _phtm_load_key(path)
    return x25519.X25519PublicKey.from_public_bytes(raw), raw

def _phtm_load_private_key(path):
    raw = _phtm_load_key(path)
    return x25519.X25519PrivateKey.from_private_bytes(raw), raw
...
def _phtm_encrypt_hybrid(data: bytes, recipient_public_raw: bytes) -> bytes:
    ...
    wrap_key = HKDF(
        ...
        length=32,
        salt=None,
        info=b"PHTM-X25519-WRAP",
    ).derive(shared)

    session_key = _secrets.token_bytes(_PHTM_KEY_SZ)
    wrap_nonce = _secrets.token_bytes(12)
    wrapped_session = AESGCM(wrap_key).encrypt(wrap_nonce, session_key, None)
    payload = _phtm_encrypt_3layer(data, session_key)
    return b"PHTM3L2" + eph_pub_raw + wrap_nonce + wrapped_session + payload

PHRAW1_MAGIC = b"PHRAW1"

def _unpack_plain_with_ext(blob: bytes) -> tuple[str, bytes]:
    """Decode PHRAW1 payload. Fallback to raw bytes if header is missing."""
    if len(blob) >= 7 and blob[:6] == PHRAW1_MAGIC:
        ext_len = blob[6]
        start = 7
        end = start + ext_len
        if 0 <= ext_len <= 31 and end <= len(blob):
            ext = blob[start:end].decode("ascii", errors="ignore").strip().lower()
...
def _find_ffmpeg_exe() -> str | None:
    ...
    # Common WinGet install location for Gyan.FFmpeg
...
def _convert_opus_to_wav(src_path: Path, dst_path: Path) -> None:
    ...
    cmd = [
        ...
        "-i", str(src_path), str(dst_path)
    ]
    proc = subprocess.run(cmd, text=True, capture_output=True, check=False, timeout=120)
    if proc.returncode != 0:
        err = (proc.stderr or "").strip()
        raise ValueError(f"ffmpeg opus->wav failed: {err}")
def _phtm_decrypt_hybrid(blob: bytes, private_key_raw: bytes) -> bytes:
    if len(blob) < 7 + 32 + 12 + 16 + 12 + 16:
        raise ValueError("Encrypted payload too short")
    if blob[:7] != b"PHTM3L2":
        raise ValueError("Unsupported payload magic (need PHTM3L2)")

    pos = 7
    eph_pub_raw = blob[pos:pos+32]; pos += 32
    wrap_nonce = blob[pos:pos+12]; pos += 12
    wrapped_session = blob[pos:pos+48]; pos += 48
    payload = blob[pos:]

    priv = x25519.X25519PrivateKey.from_private_bytes(private_key_raw)
    ...
    session_key = AESGCM(wrap_key).decrypt(wrap_nonce, wrapped_session, None)
    return _phtm_decrypt_3layer(payload, session_key)
...
def scan_phantoms(known=_KNOWN_IPS, timeout=3):
    ...
    Bn fix ny chu c li JSON t ESP32 kiu:
        "sd_total":520192 KB
        "sd_used":105762 KB
    v JSON chun khng cho ch KB nm ngoi du nhy.
    Nu gp li , app s t b hu t KB  vn nhn Phantom ONLINE.
    """
    import re
    ...
    def _fix_bad_json(raw: str) -> str:
        raw = (raw or "").strip()
        # Sa s c hu t KB khng nm trong chui:
    ...
    def _check(ip, name):
        try:
            ...
            # Nu ESP32 c tr ap_ssid/node th u tin tn thc t t thit b.
            ...
            # In ra console  debug, khng lm crash GUI.
            try:
...
class EncryptPage(ctk.CTkFrame):
    ...
    def _browse_key(self):
        ...
        if not path:
            return
        try:
            _, pub_raw = _phtm_load_public_key(path)
            pub_fp = hashlib.sha256(pub_raw).hexdigest()
            self._key_path = path
            self._key_bytes = pub_raw
            ...
            self._log(f"Public key loaded: {name}  [{last4}]")
            self._reset_bundle()
    ...
    def _generate_key(self):
        messagebox.showinfo(
            ...
            "Please generate key pair with phtm_key_gui.py and load public.key here.",
        )
    ...
    def _on_scan_result(self, results):
        ...
        if nm == "Phantom-1":
            ...
            # Phantom-2 hin th y ht Phantom-1 (icon pin + BAT xx%)
            conn_text = f"{nm}  ONLINE    BAT {batt_text}"
            ip_text = f"{ip}    KEY {l4}    Battery: {batt_icon} {batt_text}"
            try: self._app._conn_batt_icon.configure(text=batt_icon, text_color=batt_color)
            except: pass
    ...
    def _sync_progress(self, step):
        """Animate the overall progress bar 0100% during sync."""
        if step > 40: return
    ...
    def _do_encrypt(self):
        ...
        files = list(self._selected_files); public_key_raw = self._key_bytes
        master = hashlib.sha256(public_key_raw).digest()
        ...
        def _run():
            ...
            try:
                with tempfile.TemporaryDirectory(prefix="phantom_enc_") as tmpdir:
                    ...
                    enc_bytes = _phtm_encrypt_hybrid(zfld_bytes, public_key_raw)
                ...
                if self._active_ip:
                    ...
                    self._log_msg("  No device connected  skipping auto-upload")
            except Exception as e:
...
class DecryptPage(ctk.CTkFrame):
    ...
    def _dec_open_output(self):
        ...
        target = Path.home() / "Documents" / "Phantom" / "output" / "return_user"
    ...
    def _dec_start(self):
        ...
        def _run():
            ...
            try:
                ...
                _, private_key_raw = _phtm_load_private_key(key_p)
                master = hashlib.sha256(private_key_raw).digest()
            except Exception as e:
            ...
            #  Step 1: decrypt payload bytes directly 
            try:
                plain_bytes = _phtm_decrypt_hybrid(payload, private_key_raw)
                plain_ext, plain_content = _unpack_plain_with_ext(plain_bytes)
            except Exception as e:
            ...
            self._dec_log_msg(f"  Decrypted: {len(plain_content):,} bytes ext={plain_ext or '(raw)'}")

            #  Step 2: write decrypted bytes based on original extension
            os.makedirs(out_d, exist_ok=True)
            try:
                ext = plain_ext if plain_ext else ".binraw"
                if ext == ".opus":
                    opus_path = Path(out_d) / f"{Path(bin_p).stem}.opus"
                    wav_path = Path(out_d) / f"{Path(bin_p).stem}.wav"
                    with open(opus_path, "wb") as f:
                        f.write(plain_content)
                    try:
                        _convert_opus_to_wav(opus_path, wav_path)
                        try:
                        ...
                        self._dec_log_msg("  WARN: ffmpeg not found, keep .opus output")
                        result_path = opus_path
                else:
                    result_path = Path(out_d) / f"{Path(bin_p).stem}{ext}"
                    with open(result_path, "wb") as f:
                        f.write(plain_content)
                files_out = [result_path]
            except Exception as e:
                self._dec_log_msg(f"  WRITE ERROR: {e}")
                self.after(0, lambda: self._dec_status.configure(
                    text=f"Error: {e}", text_color=C_RED))
                self.after(0, lambda: self._dec_btn.configure(state="normal"))
                return
